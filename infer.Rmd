---
title: "My solution to gretting to know infer"
...
library( infer )          # The package this vignette is about ...
library( cowplot )        # For professional statistical grapgics ...


###############################################################################
# We get one of the data sets provided with the infer package.
############################################################################### 

data( gss )                         # Load the gss data.frame  available ...
 
glimpse( gss )                       # Display gss variables and data types...
```


Each row is an individual survey response, containing some basic demographic information on the respondent as well as some additional variables. See the [Appendix]() for more information on the variables included and their source. Note that this data (and our examples on it) are for demonstration purposes only, and will not necessarily provide accurate estimates unless weighted properly. For these examples, let’s suppose that this dataset is a representative sample of a population we want to learn about: American adults.  
...
To generate a null distribution for the independence of two variables, we could also randomly reshuffle the pairings of explanatory and response variables to break any existing association. For instance, to generate 1000 replicates that can be used to create a 
null distribution under the assumption that political party affiliation is not affected by age:
...
point_estimate <- gss %>%               # Compute point estimate
  specify(response = hours) %>%
    calculate(stat = "mean")  

point_estimate                           # Display point estimate
...
null_dist <- 
  gss %>%                           # Create null distribution
    specify (response = hours ) %>%
      hypothesize(null = "point", mu = 40 ) %>%
        generate(reps = 1000, type = "bootstrap") %>%
          calculate(stat = "mean")

  null_dist                          # Veify the ull distribution exists
```
...
I, Dr. Craig Slinkman do not like the default standard ggplot output. So I usually added the ##theme## cowplot to my ggplot specifications.  We can do this with the ##visualize()## function as follows.
...
p_value <- null_dist %>%                   # Get a two-tailed p-value
...
## Appendix: GSS data description

The General Social Survey is a high-quality survey which gathers data on American society and
opinions, conducted since 1972. This data set is a sample of 500 entries from the GSS, including
demographic markers and some economic variables. Note that this data is included for demonstration only, and should not be assumed to provide accurate estimates relating to the GSS. However,  due to the high quality of the GSS, the unweighted data will approximate the weighted data in some analyses.

### Keywords    [datasets](https://www.rdocumentation.org/search/keywords/datasets)
...
variables <- names(gss )                # Get column names ...
types     <- sapply( gss,               # Get column types 
                     typeof ) 

################################################################################
# We manually need to enter the documentation of the data object although as
# a future project one might want to investiugate using the R documentation
# To obtain this information.
################################################################################

descriptions <- 
  c( "Year respondent was surveyed",
...
code_book <-
  tibble( variables, types, descriptions )
...
## Source
  ...
  https://www.rdocumentation.org/packages/infer/versions/0.5.3/topics/gss
